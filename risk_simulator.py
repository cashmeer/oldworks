...
number_of_trades = 50#int(len(r_value_list))
#print(number_of_trades)

number_of_simulations = 10

min_risk_to_simulate = 0.01
max_risk_to_simulate = 0.05
...
def big_simulation():
    ...
    final_table = {'Init Equity': [], 'Risk': [], 'Goal %': [], 'Stop lmt %': [], 'Avg R': [], 'Avg Final Equity': [], 'Mean Final Equity': [],
                   'Median Equity': [], 'Avg Profit %': [], 'Max Lost %': [], 'Successful %': [], 'Stop Trade %': [],
                   'SUCC% - STOP%': []
                   }

    while current_risk <= max_risk_to_simulate:
        #current_risk += risk_increment_size
        print("current risk at", round(current_risk, 3))
    ...
    final_df = pd.DataFrame(final_table, columns=['Init Equity', 'Risk', 'Goal %', 'Stop lmt %', 'Avg R', 'Avg Final Equity', 'Mean Final Equity',
                                                  'Median Equity', 'Avg Profit %', 'Max Lost %', 'Successful %', 'Stop Trade %', 'SUCC% - STOP%']
                                       , index=index_list)

    final_df.to_csv('risk_only_simulation.csv', sep=',')
